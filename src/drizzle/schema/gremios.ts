...
export const gremios = pgTable("gremios", {
  id: text("id")
    .primaryKey()
...
(table) => {
  return {
    ...
    approvalDateIdx: index("gremio_approval_date_idx").on(table.approval_date),
    validityDateIdx: index("gremio_validity_date_idx").on(table.validity_date),
  };
});
